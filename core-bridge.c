...
int
tex_simple_main(tt_bridge_api_t *api, char *dump_name, char *input_file_name, time_t build_date, char isFormat)
{
    int rv;
    halt_on_error_p = 1;
    in_initex_mode = isFormat;
...
PRINTF_FUNC(1, 2) void ttstub_issue_warning(const char *format, ...) {
  ...
  va_start(ap, format);
  vsnprintf(error_buf, BUF_SIZE, format,
            ap); /* Not ideal to (ab)use error_buf here */
  va_end(ap);
...
size_t ttstub_output_write(rust_output_handle_t handle, const char *data,
                           size_t len) {
...
rust_input_handle_t ttstub_input_open(char const *path,
                                      tt_input_format_type format, int is_gz) {
...
size_t ttstub_input_seek(rust_input_handle_t handle, ssize_t offset,
                         int whence) {
  int internal_error = 0;
  size_t rv =
      TGB->input_seek(TGB->context, handle, offset, whence, &internal_error);
  if (internal_error) {
