...
static int kpse_fetch_from_network(char *name_ret) {
    ...
    name_ret[MAXAVAILABLENAMESIZE - 1] = 0;
    ...
    if (destfile) {
        ...
        if (curl) {
            snprintf(name_url, MAXAVAILABLENAMESIZE, "%s/%s", KPSE_BASE_URL,
                     name_copy);
            name_url[MAXAVAILABLENAMESIZE - 1] = 0;
            ...
            if (curl_res == CURLE_OK) {
                ...
                if (response_code == 200) {
                    ...
                    snprintf(name_404, MAXAVAILABLENAMESIZE, "%s/%s.404", tmpfolder,
                             name_copy);
                    name_404[MAXAVAILABLENAMESIZE - 1] = 0;
...
int compileLaTeX() {
    /* Compatibility Hack */
    printf("This is SwiftLaTeX, Version 2.0 (TeX Live 2019/Gentoo) (preloaded format=swiftlatex)\n");
    printf(" restricted \\write18 enabled.\n");
    printf("entering extended mode\n");
    return tex_simple_main(&ourapi, "swiftlatex.fmt", "InP0t_SwIfTLoTEx_.tex", time(0), 0);
}
...
int compileBibtex() {
    return bibtex_simple_main(&ourapi, "InP0t_SwIfTLoTEx_.aux");
}
